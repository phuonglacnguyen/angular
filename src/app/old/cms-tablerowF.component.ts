...
import { CmsTablerowGComponent } from './cms-tablerowG.component';
...
@Component({
  selector: 'app-cms-tablerowF',
  templateUrl: './cms-tablerowF.component.html',
  styleUrls: ['./cms-tablerowF.component.css']
})

@Injectable()
export class CmsTablerowFComponent { // implements AfterViewInit
    @Input() cmsPages;
    @Input() userlogin;
    @Input() currentPage;
  ...
  createImage(page) {
    if (this._cmsService.listAutoImage == true) {
      this._cmsService.createImage(page)
        .subscribe(res => this._cmsService.update_edit_image_fields(page));
    }
  }
  ...
  createThumb(page) {
      if(page.objectType == '5c0114dc962a560534e3e576') {
              this._cmsService.createImage(page)
               ...
               this._cmsService.createThumb(page._id)
   ...
   imageInfo(page) {
       if(page.objectType == '5bf9d60fe2776742d80831e5') {
              this._cmsService.getImageInfo(page)
