import { Component, OnInit, Input, Output, EventEmitter, OnChanges, Directive, ElementRef, ViewChild  } from '@angular/core';
...
//import { canvas, faceDetectionNet, faceDetectionOptions, saveFile } from './../../face-api/commons';

//import { Tensor } from '@tensorflow/tfjs-node';
//import { Canvas } from 'canvas';
//import { FaceDetection }  from 'face-api.js';
//const canvass = { Canvas, Image, ImageData }
...
export class UserProfileComponent implements OnInit {
  ...
  async faceR() { 
    ...
    this._userService.userfacerec(this._userService.userSelected._id)
  ...
  createForm() {
      this.userForm = this.fb.group({
          _id: new FormControl(this._userService.userSelected._id, [Validators.required] ),
          first_name: new FormControl(this._userService.userSelected.first_name, [Validators.required, Validators.minLength(2)]),
          last_name: new FormControl(this._userService.userSelected.last_name, [Validators.required, Validators.minLength(2)]),
          email: new FormControl(this._userService.userSelected.email, [Validators.required, Validators.email]),
          telephone: new FormControl(this._userService.userSelected.telephone as string, [Validators.required, Validators.minLength(10)]),
          secretLairs: this.fb.array([]),
          secretImages: this.fb.array([]),
          secretFavorites: this.fb.array([]),
          secretUrlHistory: this.fb.array([]),
          secretMovies: this.fb.array([]),
          password: new FormControl(this._userService.userSelected.password, [Validators.required, Validators.minLength(6), Validators.maxLength(60)]),
          ...
          userRole: new FormControl(this._userService.userSelected.userRole, [Validators.required]),
      ...
      var asdf = this._userService;
        ...
        if(this._userService.userSelected.urlHistory != undefined) {
          ...
          //console.log(this._userService.userSelected.urlHistory);
  ...
  setFavorites(favorites: Treeid[]) {
   ...
   setUrlHistory(urlHistory: Treeid[]) {
        const urlHistoryFGs = urlHistory.map(urlHistory => this.fb.group(urlHistory));
  ...
  onSubmit() {
    ...
    if (this._userService.userSelected._id != '') {
      this.updateUserEvent.emit({ original: this.user, edited: this._userService.userSelected });
      //console.log('update');
    } else {
      //console.log('insert');
  ...
  getUser() {
      this._userService.getUser(this._userService.userSelected)
  ...
  passgender(name, vale) {
   ...
   prepareSaveUser(): User {
      const formModel = this.userForm.value;
       ...
       const secretImagesDeepCopy: MediaImage[] = formModel.secretImages.map(
        ...
        const secretMoviesDeepCopy: Treeid[] = formModel.secretMovies.map(
         ...
         const secretMusicAlbumDeepCopy: Treeid[] = formModel.secretmusicAlbums.map(
          (treeid: Treeid) => Object.assign({}, treeid)
        );
      ...
      const saveUser: User = {
          ...
          songs: secretMoviesDeepCopy,
          musicAlbum: secretMusicAlbumDeepCopy,
          songs_tmp : secretMoviesDeepCopy,
          musicAlbum_tmp: secretMusicAlbumDeepCopy,
          songs_original : secretMoviesDeepCopy
      };
  ...
  delete_mediaimage(index) {
      var confirmText = "Are you sure you want to delete " + this._userService.userSelected.images[index].mediaTitle + " media ?";
      if (confirm(confirmText)) {
          this.secretImages.controls.splice(index, 1);
          //delete this._userService.userSelected.images[index];
          var fname = this._userService.userSelected.images[index].mediaTitle + '.' + this._userService.userSelected.images[index].mediaExtension;
  ...
  detect(URL) {
      ...
      img.onload = function () {
        ...
        (<HTMLInputElement>document.getElementById('mediaHeight')).title = height+'';
        (<HTMLInputElement>document.getElementById('mediaWidth')).title = width+'';
      };
  };
  ...
  fileChangeEvent(fileInput: any) {
      if (fileInput.target.files && fileInput.target.files[0]) {
          ...
          if (extension != 'jpg' && extension != 'gif' && extension != 'png' && extension != 'svg') {
              ...
              setTimeout(() => {
                  var width = (<HTMLInputElement>document.getElementById('mediaWidth')).title;
                  var height = (<HTMLInputElement>document.getElementById('mediaHeight')).title;
                  this.test(type, fname, size, lastmod, extension, width, height);
              }, 1000);
              document.getElementById('saveImageButton').setAttribute('class', 'btn btn-primary');
  ...
  fileUpload() {
        ...
        this._userService.upload(formData)
    ...
    uploadResult(result) {
      if (result == true) {
        //this._messageService.SetAlertSuccess('Upload succesful');
        //(<HTMLInputElement>document.getElementById(fileId)).value = '';
        //(<HTMLInputElement>document.getElementById('button_' + fileId)).classList.add('hidden');
      } else {
        //this._messageService.SetAlertWarning('Upload failed');
      }
    }
                ...
                if (this.filesToUpload.length > 0) {
                    for (let i = 0; i < fileInput.target.files.length; i++) {
                      ...
                      if (extension == 'mp4') {
                        ...
                        var img = new Image();
                        img.src = url;
                        document.getElementById('content_filetypeiuser').innerHTML += '<div><a class="thumbnail"><img src="' + url + '" alt="" /></a><br /><div class="padding_10">Filename: ' + fname + '.' + extension + '<br />Filesize: ' + size + '<br />Resolution:<span class="res"></span></div>';
                        ...
                        document.getElementById('filetypeiuser').classList.remove('hidden');
                      } else if (extension == 'mpg' || extension == 'avi' || extension == 'wmv' || extension == 'mkv') {
                            var cont = `<object id="video_mp4" width="100%" height="100%" classid="CLSID:6BF52A52-394A-11d3-B153-00C04F79FAA6" standby="Loading Media Player..."> 
                                        ...
                                        <embed wmode="transparent" type="application/x-ms-wmp" pluginspage="http://www.microsoft.com/Windows/MediaPlayer/" src="` + url +`" width="100%" autostart="0" ShowControls="1" DisplaySize="4" ></embed> 
                                        </object>`;
                            document.getElementById('filetypeiuser').classList.remove('hidden');
                            document.getElementById('content_filetypeiuser').innerHTML = cont;
                        } else {
                            document.getElementById('filetypeiuser').classList.remove('hidden');
                            document.getElementById('content_filetypeiuser').innerHTML = '<a class="thumbnail"><img src="/api/images/filetypeicons/' + extension + '.png" height="48" width="48" alt="" /></a>';
                        }
                    }
                }
      } else {
